const CONTAINER_ID = 'svelte-inspector-container';
const inspectorOptions = {};
const KEY_IGNORE = 'data-svelte-inspector-ignore';
...
function getElementPosition() {
  const scriptTag =
    ...
    document.querySelector(
      'script[src*="virtual-svelte-inspector-path-load.js"]',
    );
...
const getTargetNode = (e) => {
  ...
  const match = getData(targetNode) ?? {};
...
const openInEditor = (baseUrl, file, line, column) => {
  const promise = fetch(
    `${baseUrl}/__open-in-editor?file=${PATH_ROOT}/${file}:${line}:${column}`,
    {
      mode: 'no-cors',
    },
  );
...
function load() {
  ...
  const baseUrl = () => {
    ...
    const protocol = inspectorOptions.serverOptions?.https
      ? 'https:'
      : importMetaUrl?.protocol;
    const hostOpts = inspectorOptions.serverOptions?.host;
    const host =
      hostOpts && hostOpts !== true ? hostOpts : importMetaUrl?.hostname;
    const port =
      hostOpts && hostOpts !== true
        ? inspectorOptions.serverOptions?.port
        : importMetaUrl?.port;
    const baseUrl = isClient
      ? inspectorOptions.openInEditorHost || `${protocol}//${host}:${port}`
      : '';
    return baseUrl;
  };
  ...
  const floatsStyle = (position) => {
    let margin = 10;
    let x = position.x + position.width / 2;
    let y = position.y + position.height + 5;
    let floatsWidth = overlay?.clientWidth ?? 0;
    let floatsHeight = overlay?.clientHeight ?? 0;
  ...
  const sizeIndicatorStyle = (position) => {
    return {
      left: `${position.x}px`,
      top: `${position.y}px`,
  ...
  const update = () => {
    if (active && overlayVisible) {
      ...
      overlay.style.left = floatsStyle(position).left;
      overlay.style.top = floatsStyle(position).top;
      indicator.style.left = sizeIndicatorStyle(position).left;
      indicator.style.top = sizeIndicatorStyle(position).top;
      indicator.style.width = sizeIndicatorStyle(position).width;
      indicator.style.height = sizeIndicatorStyle(position).height;
      content.innerHTML = linkParams.file;
    } else {
      overlay.style.display = 'none';
      indicator.style.display = 'none';
      content.innerHTML = '';
